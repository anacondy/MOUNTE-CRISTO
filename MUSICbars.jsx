...
import { 
  ...
  Terminal, Shield, ShieldAlert, Volume2, VolumeX, Atom 
} from 'lucide-react';
...
const VolumeFeedback = ({ isMuted, visible }) => {
  if (!visible) return null;
  
  return (
    <div className="absolute inset-0 z-[60] flex items-center justify-center pointer-events-none">
      <div className="bg-[#1a120b]/90 p-8 rounded-full border-2 border-[#d44d38] backdrop-blur-sm animate-[ping_0.5s_cubic-bezier(0,0,0.2,1)_1]">
        {isMuted ? (
          <VolumeX size={64} className="text-[#d44d38]" />
        ) : (
          <Volume2 size={64} className="text-[#eaddcf]" />
        )}
      </div>
    </div>
  );
...
const CustomAudioPlayer = ({ url }) => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (audioContextRef.current && audioContextRef.current.state !== 'closed') {
        // We don't close it here strictly to avoid re-creation issues on hot reloads, 
        // but in prod you might.
      }
    };
  }, [url]);
  ...
  const drawVisualizer = () => {
    ...
    const renderFrame = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        ...
        // Color: "Dried Blood" Red with some gradient logic
        const red = 212; // #d4
        const green = 77; // #4d
        const blue = 56; // #38
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center relative bg-[#150f0a] overflow-hidden">
      <VolumeFeedback isMuted={isMuted} visible={showFeedback} />
...
const FileViewer = ({ file, onClose }) => {
  ...
  // Construct JSX Runner Blob
  const getRunnerSrc = () => {
    if (isHtml) return url;
    if (isJsx && codeContent) {
      // Create a complete HTML document that compiles JSX on the fly
      const jsxRunner = `
        <!DOCTYPE html>
        <html>
        <head>
          ...
          <style>body { margin: 0; font-family: sans-serif; color: #eaddcf; background: #0d0907; } </style>
        </head>
        <body>
          <div id="root"></div>
          <script type="text/babel">
            ...
            // Try to find default export or just render if it's a simple snippet
            // If the user code renders to root, great. If not, we try to catch simple components.
          </script>
        </body>
        </html>
      `;
  ...
  return (
    <div className="relative w-full h-full flex flex-col bg-black/50 backdrop-blur-md rounded-sm border border-[#3e3228] overflow-hidden animate-[fadeIn_0.3s_ease-out]">
           ...
           {(isHtml || isJsx) && (
             <div className="flex items-center gap-2 ml-4 px-2 py-1 bg-[#261f1a] rounded border border-[#3e3228]">
                ...
                >
                  ...
                  {runCode ? 'View Source' : (isJsx ? 'Compile & Run' : 'Run Code')}
                </button>
             </div>
           )}
        </div>
      ...
      <div className="flex-1 overflow-hidden flex items-center justify-center bg-[#0d0907] relative">
        ...
        {(isHtml || isJsx) && runCode && (
           <iframe 
             title="preview"
             src={isHtml ? url : runnerUrl} 
             className="w-full h-full bg-white"
             // Relaxing sandbox for JSX execution as it needs eval (Babel)
             sandbox="allow-scripts allow-modals allow-popups allow-forms allow-same-origin"
           />
        )}

        {/* Code Viewer */}
        {((isCode && !isHtml && !isJsx) || ((isHtml || isJsx) && !runCode)) && (
