...
// Mute/Volume Feedback Overlay - REFINED (Text Only & Red Volume)
...
const CustomAudioPlayer = ({ url }) => {
  ...
  // Improved Dependency Array to ensure Visualizer restarts on switch
  useEffect(() => {
    if (isPlaying) {
  ...
  }, [isPlaying, url]); // Added 'url' here to force restart on file switch
...
const CustomVideoPlayer = ({ url }) => {
  ...
  // Robust Auto-play & Load sequence to fix blank screens
  useEffect(() => {
    if(videoRef.current) {
        videoRef.current.load(); // Forces reload to handle codec/container changes
  ...
  return (
    <div className="relative group w-full h-full flex items-center justify-center bg-black overflow-hidden">
      ...
      <video 
        ...
        src={url}
        playsInline 
        className="max-h-full max-w-full shadow-2xl cursor-pointer bg-black" // Added bg-black to avoid white flashes
        onClick={togglePlay}
